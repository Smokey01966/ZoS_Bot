module.exports.run = async (bot, message, args) => {
    ...
    if(args.length !== 2){sender.send(`Incorrect number of arguments for this command\nThe format is:\n${botConfig.prefix}addbonustickets|(Gw2 Account)|(Number of Tickets)`);return;}//set number
    ...
    lotoPropRef=firebase.database().ref("lottery/properties/bonus")
    lotoRecRef=firebase.database().ref(`lottery/record/bonus/${gw2Account}`)
            ...
            numberOfTicketBought=Number(snapshot.child("Number of Tickets Bought").val())
                ...
                lotoRecRef.once("value").then(function(snapshot){
                    ...
                    if(numberOfTicketsPerPerson==="Unlimited"){
                        ...
                        sender.send(`${gw2AccountIn} has had the bonus ticket number(s) ${lotoTicketsGiven.join(" ")} assigned to them`)
                    }else{
...
module.exports.help = {
    name: "addbonustickets"//Change to name of command
}
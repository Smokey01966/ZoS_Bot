module.exports.run = async (bot, message, args) => {
    ...
    .setTitle("Close Poll Command")
    .setColor("#3d7d05")
    .addField("Format", `${botConfig.prefix}closepoll|(Poll Number)`)
    .addField("Note on Permissions", `Only the poll owner or a ${ranksConfig.data.elevatedOfficerRanks.join(" or ")} can edit a poll`)
    .addField("Poll Number", "The number of the poll you wish to close")
    sender.send(messageEmbed);
}
module.exports.help = {
    name: "closepollhelp"//Change to name of command
}
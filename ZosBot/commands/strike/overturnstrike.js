module.exports.run = async (bot, message, args) => {
    ...
    dataRef.orderByChild("Discord Account").equalTo(discordAccount).once("value", function(snapshot){
        ...
        newNote = `Overturned by (${senderGw2AccountDis} on ${moment().tz(timeZone).format().slice(0,10)}):\n${note}`
        noteRef.once("value", function(snapshot){
            ...
            strikeRef.once("value", function(snapshot){
                if(snapshot.child("Status").val() !== "Appealed"){sender.send("This strike is not eligible to be overturned");return;}
                ...
                statusRef.set("Overturned")
                dateChangeRef.set(moment().tz(timeZone).format().slice(0,10))
                sender.send("Strike Overturned")
            })
        })
    })         
}
module.exports.help = {
    name: "overturnstrike"//Change to name of command
}